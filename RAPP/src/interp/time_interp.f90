...
subroutine time_interp()
   ...
   !---------------------------------------------------------------------------------------!
   ! STEP 1.  Fill the time array for the second grid, using the first grid as reference.  !
   !---------------------------------------------------------------------------------------!
   st1loop: do t2=1,sstp(2)
      t1   = 1 + (t2-1)/radratio
      it   = mod((t2-1),radratio)

      fracday = this_time(t1,1)%fracday + it * deltat
      ...
      !------------------------------------------------------------------------------------!
      this_time(t2,2)%fracday = mod(fracday,1.)
      this_time(t2,2)%doy     = this_time(t1,1)%doy + int(fracday)
      this_time(t2,2)%year    = this_time(t1,1)%year

      call fill_time_info(this_time(t2,2))
   end do st1loop

   !---------------------------------------------------------------------------------------!
   ! STEP 2.  Find the normalised factors for each bin of time, then perform interpol-     !
   !          ation.                                                                       !
   !---------------------------------------------------------------------------------------!
   st2loop: do t1=1,sstp(1)
      write (unit=*,fmt='(a,1x,2a)')                                                       &
                                '         [|] Interpolating:',this_time(t1,1)%timestr,'...'

      yloop: do yy=1,mygauss
         xloop: do xx=1,mxgauss
            !----- Get normalisation factors for this time. -------------------------------!
            call calculate_normalised_factors(this_time(t1,1),grid_g(1)%lon(xx,yy)         &
