...
#==========================================================================================#
#==========================================================================================#
#     Commercial volume, based on Chambers et al. (2001)                                   #
#                                                                                          #
# Chambers, J. Q. and co-authors, 2001: Tree damage, allometric relationships, and above-  #
#     ground net primary production in central Amazon forest.  Forest Ecol. Manag.,        #
#     152(1-3), 73-84, doi:10.1016/S0378-1127(00)00591-0
#------------------------------------------------------------------------------------------#
dbh2vcomm <<- function(dbh,hcomm,ipft){

   #----- Find total and commercial height. -----------------------------------------------#
   if (missing(hcomm)){
      htot  = dbh2h(dbh=dbh,ipft=ipft)
      hcomm = h2crownbh(height=htot,ipft=ipft)
   }#end if (missing(hcomm))
   #---------------------------------------------------------------------------------------#

   #---------------------------------------------------------------------------------------#
   #     Apply taper function to estimate bole volume.                                     #
   #---------------------------------------------------------------------------------------#
   v1  = 0.091
   v0  = 0.01 / (1.3^-v1)
   a0  = 0.25 * pi * v0^2 / (1. - 2*v1)
   a1  = 1 - 2*v1
   ans = a0 * dbh^2 * hcomm^a1
   #---------------------------------------------------------------------------------------#

   return(ans)
}#end function dbh2vcomm
#==========================================================================================#
#==========================================================================================#
