...
#------------------------------------------------------------------------------------------#
assign.pft <<- function(datum,path=srcdir,verbose=FALSE){
   ...
   #---------------------------------------------------------------------------------------#
   #     Initialise genus with the actual one, but replace those not find in the look-up   #
   # table.                                                                                #
   #---------------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------------#
   #    Initialise use.scientific with the actual scientific name.  We will replace those  #
   # that are not found in the look-up table for traits.                                   #
   #---------------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------------#
   #    Retrieve scientific name.  In case the scientific name was not used in the cluster #
   # analysis, randomly select one name from the same genus, or from the same family in    #
   # case the genus is not available.  In case nothing is known, make a completely random  #
   # selection.                                                                            #
   #---------------------------------------------------------------------------------------#
   ...
   sci.genus.mean  = sci.genus.mean [! duplicated(sci.genus.mean$scientific ),,drop=FALSE]
   sci.family.mean = sci.family.mean[! duplicated(sci.family.mean$scientific),,drop=FALSE]
   sci.loose.mean  = sci.loose.mean [! duplicated(sci.loose.mean$scientific ),,drop=FALSE]
   ...
   for (s in seq_along(sci.genus.mean$scientific)){
      ...
      #------------------------------------------------------------------------------------#
      #      List of scientific names from the same genus that are available in the look   #
      # up table.  We also include scientific names that exist in the source data set,     #
      # belong to the same genus, and are found in the look-up table, to increase chances  #
      # of selecting a scientific name observed in the source data itself.                 #
      #------------------------------------------------------------------------------------#
   ...
   for (s in seq_along(sci.family.mean$scientific)){
      ...
      #------------------------------------------------------------------------------------#
      #      List of scientific names from the same family that are available in the look  #
      # up table.  We also include genera that exist in the source data set, belong to the #
      # same family, and are found in the look-up table, to increase chances of selecting  #
      # a genus observed in the source data itself.                                        #
      #------------------------------------------------------------------------------------#
   ...
   for (s in seq_along(sci.loose.mean$scientific)){
      ...
      # include species that exist in the source data set are found in the look-up table,  #
      # to increase chances of selecting a species observed in the source data itself.     #
      #------------------------------------------------------------------------------------#
      d.sel    = yes.sci
   ...
   for (tt in which(trait$cluster)){
      ...
      for (s in seq_along(un.scientific)){
         ...
         #---------------------------------------------------------------------------------#
         #     Obtain sample.  This ensures that imputed data will not be a unique value,  #
         # whereas well-constrained genera will have no variability (so all plants in the  #
         # same genus will have the same number).                                          #
         #---------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------------#
   #      For each row, we compute the dissimilarity matrix with the medoid, then pick the #
   # least dissimilar one.                                                                 #
   #---------------------------------------------------------------------------------------#
