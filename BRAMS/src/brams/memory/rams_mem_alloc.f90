...
subroutine rams_mem_alloc(proc_type)
  ...
  integer :: ng,nv,imean, na   !,ntpts  ! variable never used
!!$  INTEGER :: ng,nv,imean,ntpts
  ...
  write (unit=*,fmt=*) ' + Basic allocation;'
...
![MLO - Added in the BRAMS-stilted style, so we can run Grell's shallow cumulus without
!       running CATT
  ! Allocate Cuparm variables data type
  write (unit=*,fmt=*) ' + Cuparm allocation;'
  allocate(cuparm_g(ngrids),cuparmm_g(ngrids))
  allocate(cuparm_g_sh(ngrids),cuparmm_g_sh(ngrids)) !ML - for Grell shallow cumulus
  do ng=1,ngrids
     call nullify_cuparm(cuparm_g(ng))
     call nullify_cuparm(cuparmm_g(ng))
     call nullify_cuparm(cuparm_g_sh(ng)) !ML - for Grell shallow cumulus
     call nullify_cuparm(cuparmm_g_sh(ng)) !ML - for Grell shallow cumulus
     call alloc_cuparm(cuparm_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
     call alloc_cuparm(cuparm_g_sh(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng) !ML - for Grell shallow cumulus 
     if (imean == 1) then  
        call alloc_cuparm(cuparmm_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
        call alloc_cuparm(cuparmm_g_sh(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng) !ML - for Grell shallow cumulus
     elseif (imean == 0) then
        call alloc_cuparm(cuparmm_g(ng),1,1,1,ng)
        call alloc_cuparm(cuparmm_g_sh(ng),1,1,1,ng) !ML - for Grell shallow cumulus
     endif
...
!MLO]
  ...
  write (unit=*,fmt=*) ' + Leaf allocation;'
  ...
  write (unit=*,fmt=*) ' + Micro allocation;'
  ...
  write (unit=*,fmt=*) ' + Turb allocation;'
...
!!MLO]
  ...
  write (unit=*,fmt=*) ' + Varinit allocation;'
  ...
  write (unit=*,fmt=*) ' + Grid allocation;'
  ...
  write (unit=*,fmt=*) ' + Scalar allocation;'
  ...
  write (unit=*,fmt=*) ' + Tend allocation;'
  ...
  write (unit=*,fmt=*) ' + Scratch allocation;'
  ...
  if (CATT == 1) then
     ...
     write (unit=*,fmt=*) ' + CATT allocation;'
  ...
  call nullify_scratch1()
     write (unit=*,fmt=*) ' + Scratch1 allocation;'
  ...
  if ((if_adap==0).and.(ihorgrad==2)) then
     ...
     write (unit=*,fmt=*) ' + Opt_scratch allocation;'
...
!!$  if (proc_type == 0 .or. proc_type == 2) then
  ...
  if (proc_type == 0 .or. proc_type == 2 .or. proc_type == 1) then
     write (unit=*,fmt=*) ' + nest allocation;'
     do ng=1,ngrids
  ...
  if (any(nnshcu(1:ngrids) == 1)) then
     write (unit=*,fmt=*) ' + Shcu allocation;'
...
!
! Verifying if the allocation is necessary in any grids
![MLO - Changed the way it is done here: the use of shallow cumulus no longer depends on catt
  do ng=1, ngrids
     if (nnqparm(ng) == 2 .or. nnshcu(ng) == 2) Alloc_Grell_Flag = 1
  enddo
  ! If (Alloc_Grell_Flag == 1) start allocation
  if (Alloc_Grell_Flag == 1) then
     write (unit=*,fmt=*) ' + Grell allocation;'
...
![MLO -- For shallow cumulus
     ...
     allocate(grell_g(ngrids),grellm_g(ngrids))
     allocate(grell_g_sh(ngrids),grellm_g_sh(ngrids))
     ! Allocating the scratch array in a new fashion
...
!MLO]
     do ng=1,ngrids
        if (nnqparm(ng) == 2 .or. nnshcu(ng) == 2) then
           ...
           call alloc_grell(grell_g_sh(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
           call alloc_scratch1_grell(sc1_grell_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),maxiens)
           call zero_scratch1_grell(sc1_grell_g(ng))
           if (imean == 1) then
              call alloc_grell(grellm_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
              call alloc_grell(grellm_g_sh(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
           elseif (imean == 0) then
              call alloc_grell(grellm_g(ng),1,1,1,ng)
              call alloc_grell(grellm_g_sh(ng),1,1,1,ng)
           endif
     ...
     call zero_scratch3_grell_sh() !LFR
      ...
      Flag_Grell = 2
     ! Selecting the closure for deep convection
     if (any(nnqparm == 2)) then
        ...
        icoic = 1 ! Just to define something
     endif
     if (any(nnshcu == 2)) then
        !Selecting the closure for shallow convection
...
![MLO - Allocate mass variables data type
  write (unit=*,fmt=*) ' + Mass allocation;'
  allocate(mass_g(ngrids), massm_g(ngrids))
  do ng=1, ngrids
    call nullify_mass(mass_g(ng))
    call nullify_mass(massm_g(ng))
    call alloc_mass(mass_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
    if (imean == 1) then
      call alloc_mass(massm_g(ng),nmzp(ng),nmxp(ng),nmyp(ng),ng)
    else if (imean == 0) then
      call alloc_mass(massm_g(ng),1,1,1,ng)
    end if  
...
!ML]
  ...
  if (CATT == 1) then
     write (unit=*,fmt=*) ' + CATT extras allocation;'
  ...
  if (ilwrtyp==4 .or. iswrtyp==4) then
     write (unit=*,fmt=*) ' + CARMA allocation;'
  ...
  if (TEB_SPM==1) then
     ...
     !
     write (unit=*,fmt=*) ' + Tebc allocation;'
     ...
     !
     if(iteb==1)then
        write (unit=*,fmt=*) ' + Teb(2) allocation;'
  ...
  if (proc_type == 0 .or. proc_type == 2 .or. proc_type == 1) then
     write (unit=*,fmt=*) ' + Lite allocation;'
