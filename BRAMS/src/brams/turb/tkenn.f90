...
subroutine nakanishi(m1,m2,m3,m4,ia,iz,ja,jz,jd,level,tkep,tket,vt3dd,vt3de,vt3dh,vt3di    &
                    ,vt3dj,scr1,rtgt,theta,rv,dn0,up,vp,vegz0,patchz0,tstar,ustar          &
...
use turb_constants, only:  nna1,nna2,nnb1,nnb2,nnc1,nnc2,nnc3,nnc4,nnc5                    &
                          ...
                          ,nngama1,nngama2,nnf1,nnf2,nnrf1,nnrf2,nnri1,nnri2,nnri3,nnrfc   &
                          ,nnce1a,nnce1b,nnce2,nnce3,nnce4,nncr1                           &
...
!------------------------------------------------------------------------------------------!
integer  , intent(in)                               :: m1,m2,m3,m4,ia,iz,ja,jz,jd,level
...
real                   :: dzloc, rf, ri, sh2, sm2, gm2, qlevel2, oneminusalpha
...
!----- The sum is between k2w and m1-1: k1 and m1 are just boundaries ---------------------!
    sumtkz=ssum(m1-k2w,vctr32(k2w:(m1-1)),1)
    sumtk =ssum(m1-k2w,vctr33(k2w:(m1-1)),1)
!------------------------------------------------------------------------------------------!

!----- Finding the average dU/dz and dV/dz at the thermodynamic level k2 ------------------!
    du0dz=0.25*(vt3dd(k2um1,i-1,j) +vt3dd(k2u,i,j)+vt3dd(k2um1-1,i-1,j)+vt3dd(k2u-1,i,j))
    dv0dz=0.25*(vt3de(k2vm1,i,j-jd)+vt3de(k2v,i,j)+vt3de(k2vm1-1,i,j-jd)+vt3de(k2v-1,i,j))
!------------------------------------------------------------------------------------------!
...
!------------------------------------------------------------------------------------------!
      ...
      elseif (vt3dj(k,i,j) > 0 .and. vctr5(k) < 0) then
        ...
        lb = huge(nnq6) !----- Making it as close to infinity as the computer allows. -----!
      end if
...
!------------------------------------------------------------------------------------------!
!   Obtaining an estimation of the PBL height.                                             !
      ...
      convmixlay: do k=k2w+1,m1-1
         ...
         ri=g*(vctr21(k)-vctr22(k))*(vctr1(k)-vctr1(k2w))/                                 &
            (vctr22(k)*((vctr29(k)-vctr29(k2w))*(vctr29(k)-vctr29(k2w))+                   &
                       (vctr31(k)-vctr31(k2w))*(vctr31(k)-vctr31(k2w))+100.*ustarw*ustarw))
...
!------------------------------------------------------------------------------------------!
!  Case of growing turbulence, it must use the modified Level 2.5 model (Nakanishi         !
! and Niino, 2004), otherwise it just uses the original Level 2.5                          !
!------------------------------------------------------------------------------------------!
...
!------------------------------------------------------------------------------------------!
      sigw(k,i,j)= sqrt(max(vctr38(k)*(0.33333333333-2*nna1*vctr23(k)*vctr19(k)            &
