...
subroutine grell_dellabot_ensemble(mgmzp,checkmass,masstol,edt,this,p_cup,this_cup         &
   ...
   dellathis(1) = (detdo1 * .5*(thisd_cld(1)+thisd_cld(2)) + detdo2 * thisd_cld(1)         &
                  + subin * this_cup(2) - entdo * this(1) ) * g /(p_cup(2)-p_cup(1))
...
!==========================================================================================!
!==========================================================================================!
!    This subroutine takes out cloud liquid water by detrainment. Because no cloud         !
! entrainment happens and only updrafts detrain liquid water even when downdrafts are      !
! present
!------------------------------------------------------------------------------------------!
subroutine grell_dellacond_ensemble(mgmzp,kbcon,ktop,p_cup,cdu,dzd_cld,etau_cld,qlu_cld    &
                                   ,dellaqc)
   use rconstants, only : g
   ...
   real, dimension(mgmzp), intent(in)    :: dzd_cld     ! Delta-z for downdrafts;
   real, dimension(mgmzp), intent(in)    :: etau_cld    ! Normalized updraft mass flux;
   real, dimension(mgmzp), intent(in)    :: qlu_cld     ! Liquid water at updraft;
   real, dimension(mgmzp), intent(inout) :: dellaqc     ! Change of water per unit of mass

   integer                               :: k           ! Counter

   !---------------------------------------------------------------------------------------!
   !    The vertical loop is done only between the above the cloud base up to the level    !
   ! immediately below the cloud top. We have liquid water only at the cloud levels.       !
   !---------------------------------------------------------------------------------------!
   vertloop: do k=kbcon+1,ktop-1
      dellaqc(k) = g * dzd_cld(k) * cdu(k) *etau_cld(k) * .5 * (qlu_cld(k)+qlu_cld(k+1))   &
                 / (p_cup(k)-p_cup(k+1))

   end do vertloop

   !----- Special boundary condition at the top -------------------------------------------!
   dellaqc(ktop) = etau_cld(ktop) * qlu_cld(ktop) * g / (p_cup(ktop)-p_cup(ktop+1))

   return
end subroutine grell_dellacond_ensemble
!==========================================================================================!
!==========================================================================================!
...
subroutine grell_efficiency_ensemble(mkx,mgmzp,maxens_eff,k22,kbcon,ktop,edtmin,edtmax     &
                                    ,pwav,pwev,z_cup,uwind,vwind,edt_eff)
   ...
   real                                  :: zkbc_kft    ! Cloud base height
   !---------------------------------------------------------------------------------------!
   ...
   botwind = 0.5*( sqrt(uwind(kbcon   )**2 + vwind(kbcon   )**2)                           &
                 + sqrt(uwind(kbcon+ 1)**2 + vwind(kbcon+ 1)**2))

   !----- Fritsch-Chappell used wind shear in units of 1/(1000*s) -------------------------!
   vshear = 1000.*(topwind-botwind)/(z_cup(ktop)-z_cup(kbcon))
   ...
   !---------------------------------------------------------------------------------------!
   !----- Converting the height at LCL from m to thousands of feet (kft) ------------------!
...
subroutine grell_dyncontrol_ensemble(closure_type,comp_down,mgmzp,maxens_dyn,dtime         &
   ...
   real                       , intent(in)  :: edt          ! Epsilon, related to prec. eff.
   ...
   real                       , intent(in)  :: aatot        ! Cloud work function w/ forcing
   ...
   real                                     :: divisor      ! Scratch divisor
   real                       , parameter   :: tinyden=1.e-7! Small number for division
   ...
   !---------------------------------------------------------------------------------------!
   case ('kf')
      divisor     = sign(max(tinyden,abs(aatot-x_aatot)),aatot-x_aatot) + dens_curr
      upmf_dyn(1) = max(0., mbprime * dtime * aatot0 / (divisor * tscal_kf))
   !---------------------------------------------------------------------------------------!
   ...
   case ('en','nc')
      ...
      if (closure_type == 'en') then
         ...
         !----- Picking up the strongest mass flux below the cloud base (except k22) ------!
...
subroutine grell_feedback(comp_down,mgmzp,maxens_cap,maxens_eff,maxens_lsf,maxens_dyn      &
   ...
   real, dimension(mgmzp), intent(out)   :: outt        ! Change in temperature profile
   real, dimension(mgmzp), intent(out)   :: outq        ! Change in total mixing ratio
   real, dimension(mgmzp), intent(out)   :: outqc       ! Change in cloud mixing ratio
   ...
   ! is heating/cooling too much, rescale (aka cheat...)  the reference upward mass flux.  !
   ! Since max_heat is given in K/day, I will compute outt in K/day just to test the       !
   ! value and check whether it is outside the allowed range or not. After I'm done, I     !
   ! will rescale it back to K/s.                                                          !
   !---------------------------------------------------------------------------------------!
   do k=1,ktop
      outt(k) = upmf * sum(dellat_eff(k,:,:)) * inv_maxens_effcap * day_sec
   end do
   !----- Get minimum and maximum outt, and where they happen -----------------------------!
   kmin = minloc(outt,dim=1)
   kmax = maxloc(outt,dim=1)
   
   !----- If excessive heat happens, scale down -------------------------------------------!
   if (kmax > 2 .and. outt(kmax) > 2.0 * max_heat) then
      rescale = 2.0*max_heat / outt(kmax)
      upmf = upmf * rescale
      do k=1,ktop
         outt(k) = outt(k) * rescale
      end do
   end if
   !----- If excessive cooling happens, scale down ----------------------------------------!
   if (outt(kmin)  < - max_heat) then
      rescale = - max_heat/ outt(kmin)
      upmf = upmf * rescale
      do k=1,ktop
         outt(k) = outt(k) * rescale
      end do
   ...
   do k=1,2
      if (outt(k) > 0.5 * max_heat) then
         rescale = 0.5 * max_heat / outt(k)
         upmf = upmf * rescale
         do l=1,ktop
            outt(l) = outt(l) * rescale
         end do
      end if
   end do
   !----- Converting outt to K/s ----------------------------------------------------------!
   do k=1,ktop
      outt(k) = outt(k) / day_sec
   end do
   !---------------------------------------------------------------------------------------!
   ...
   !---------------------------------------------------------------------------------------!
   do k=1,ktop
      outq(k)  = upmf * sum(dellaq_eff(k,:,:) ) * inv_maxens_effcap
      outqc(k) = upmf * sum(dellaqc_eff(k,:,:)) * inv_maxens_effcap
   end do
   !---------------------------------------------------------------------------------------!
