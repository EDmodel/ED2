   ...
   jloop: do j=ja,jz
      iloop: do i=ia,iz
         ...
         fdbkloop: do icld=cldd,clds
            call grell_feedback_driver(banneron,i,j,icld,dti)
         end do fdbkloop
...
subroutine grell_cupar_static_driver(banneron,i,j,icld)
   ...
   call grell_cupar_static(comp_down(icld),comp_noforc_cldwork,checkmass,iupmethod         &
                     , maxens_cap,maxens_eff,mgmzp,cap_maxs(icld),cap_max_increment        &
                     , wnorm_max(icld),wnorm_increment,depth_min(icld),depth_max(icld)     &
                     , edtmax,edtmin,masstol,pmass_left,radius(icld),relheight_down        &
                     , zkbmax(icld),zcutdown(icld),z_detr(icld)                            &
                     ...
                     , ensemble_e(icld)%pwav_cap        , ensemble_e(icld)%pwev_cap        &
                     , ensemble_e(icld)%wbuoymin_cap    , ensemble_e(icld)%cdd_cap         &
                     , ensemble_e(icld)%cdu_cap         , ensemble_e(icld)%mentrd_rate_cap &
                     , ensemble_e(icld)%mentru_rate_cap , ensemble_e(icld)%dbyd_cap        &
                     , ensemble_e(icld)%dbyu_cap        , ensemble_e(icld)%etad_cld_cap    &
                     , ensemble_e(icld)%etau_cld_cap    , ensemble_e(icld)%rhod_cld_cap    &
                     , ensemble_e(icld)%rhou_cld_cap    , ensemble_e(icld)%qliqd_cld_cap   &
                     , ensemble_e(icld)%qliqu_cld_cap   , ensemble_e(icld)%qiced_cld_cap   &
                     , ensemble_e(icld)%qiceu_cld_cap   , i,j,icld,mynum                   )
   return
...
!------------------------------------------------------------------------------------------!
subroutine grell_feedback_driver(banneron,i,j,icld,dti)
   ...
   !---------------------------------------------------------------------------------------!
   ! A. Computing the feedback to the large-scale per se.  This will determine the forcing !
   ...
   call grell_cupar_feedback(comp_down(icld),mgmzp,maxens_cap,maxens_eff,maxens_lsf        &
                         , maxens_dyn,inv_ensdim,max_heat(icld)                            &
                         ...
                         , ensemble_e(icld)%ierr_cap      , cuparm_g(ngrid)%upmf(i,j,icld) &
                         , cuparm_g(ngrid)%dnmf(i,j,icld) , cuparm_g(ngrid)%edt (i,j,icld) &
                         , i,j,icld,mynum                 )


   !---------------------------------------------------------------------------------------!
   ! B. Resetting output variables to zero.                                                !
   !---------------------------------------------------------------------------------------!
   call azero(mzp,vctr28)

   !---------------------------------------------------------------------------------------!
   ! C. We now compute the other output variables such as mass fluxes and interesting      !
   !    levels.                                                                            !
   !---------------------------------------------------------------------------------------!
   if (banneron) write (unit=60+mynum,fmt='(3(a,1x,i5,1x))')                               &
   ...
   !---------------------------------------------------------------------------------------!
   ! D. Copy the CO2 forcing if CO2 is available.                                          !
   !---------------------------------------------------------------------------------------!
   if (co2_on) call atob(mzp,vctr28(1:mzp),cuparm_g(ngrid)%co2src(1:mzp,i,j,icld))

   !---------------------------------------------------------------------------------------!
   ! E. If the user needs the full mass flux information to drive Lagrangian models, then  !
   !    do it now.                                                                         !
   !---------------------------------------------------------------------------------------!
   if (banneron) write (unit=60+mynum,fmt='(3(a,1x,i5,1x))')                               &
   ...
   if (catt == 1) then
      if (icld == 1) then
         call grell_massflx_stats(mzp,icld,.true.,dti,maxens_dyn,maxens_lsf,maxens_eff     &
                                 ...
                                 , ensemble_e(icld)%ierr_cap  , ensemble_e(icld)%upmf_ens  &
