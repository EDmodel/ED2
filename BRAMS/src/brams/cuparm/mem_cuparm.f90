...
module mem_cuparm
  ...
  type cuparm_vars
     ...
     real, pointer, dimension(:,:,:,:) :: &
          ...
          ,cupcond                        ! ! Liquid water occurring on param. clouds.
...
! first and last cloud spectrum that will be computed by Grell parameterization, and turn  !
! off any clouds other than the deepest when running cumulus inversion.                    !
!------------------------------------------------------------------------------------------!
...
!==========================================================================================!
  subroutine alloc_cuparm(cuparm,n1,n2,n3,ng)
    ...
    !--------------------------------------------------------------------------------------!
    !   These variables should always be allocated if any cumulus parameterization was on. !
    !--------------------------------------------------------------------------------------!
    ...
    allocate (cuparm%cupcond (n1,n2,n3,nclouds))
...
!==========================================================================================!
  subroutine nullify_cuparm(cuparm)
    ...
    if(associated(cuparm%cupcond    ))  nullify (cuparm%cupcond    )
...
!==========================================================================================!
  subroutine dealloc_cuparm(cuparm)
    ...
    if(associated(cuparm%cupcond    ))  deallocate (cuparm%cupcond    )
...
!==========================================================================================!
  subroutine initialize_cuparm(cuparm)
    ...
    if(associated(cuparm%cupcond    ))  cuparm%cupcond    = 0.
...
!==========================================================================================!
  subroutine filltab_cuparm(cuparm,cuparmm,imean,n1,n2,n3,ng)
    ...
    if (associated(cuparm%thsrc))  &
         call vtables2 (cuparm%thsrc(1,1,1,1),cuparmm%thsrc(1,1,1,1)  &
         ,ng, npts, imean,  &
    ...
    if (associated(cuparm%cupcond))  &
         call vtables2 (cuparm%cupcond(1,1,1,1),cuparmm%cupcond(1,1,1,1)  &
         ,ng, npts, imean,  &
         'CUPCOND :8:hist:anal:mpti:mpt3')
    !--------------------------------------------------------------------------------------!
    ...
    if (associated(cuparm%aadn))  &
         call vtables2 (cuparm%aadn(1,1,1),cuparmm%aadn(1,1,1)  &
         ,ng, npts, imean,  &
    ...
    if (associated(cuparm%upmf))  &
         call vtables2 (cuparm%upmf(1,1,1),cuparmm%upmf(1,1,1)  &
         ,ng, npts, imean,  &
    ...
    if (associated(cuparm%zk22))  &
         call vtables2 (cuparm%zk22(1,1,1),cuparmm%zk22(1,1,1)  &
         ,ng, npts, imean,  &
         'ZK22 :9:hist:anal:mpti:mpt3')

    if (associated(cuparm%zkdt))  &
         call vtables2 (cuparm%zkdt(1,1,1),cuparmm%zkdt(1,1,1)  &
         ,ng, npts, imean,  &
         'ZKDT :9:hist:anal:mpti:mpt3')

    if (associated(cuparm%zkbcon))  &
         call vtables2 (cuparm%zkbcon(1,1,1),cuparmm%zkbcon(1,1,1)  &
         ,ng, npts, imean,  &
