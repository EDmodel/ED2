...
!------------------------------------------------------------------------------------------!
subroutine grell_cupar_output(comp_down,m1,mgmzp,maxens_cap,rtgt,zm,zt,dnmf,upmf,ierr_cap  &
                             ...
                             ,areadn,areaup,cuprliq,cuprice,i,j,icld,mynum)
   ...
   real               , intent(inout) :: zjmin     ! Downdraft originating level  [      m]
   real               , intent(inout) :: zk22      ! Updraft origin               [      m]
   real               , intent(inout) :: zkbcon    ! Level of free convection     [      m]
   ...
   if (print_debug) then
      ...
      write(unit=iun,fmt=fmti)  'KGOFF = ',kgoff
      write(unit=iun,fmt=fmti)  'ICLD  = ',icld
      write(unit=iun,fmt='(3(a,1x))') 'LEVEL','       THSRC','     OUTTHIL'
      do k=1,mkx
         kr = k + kgoff
         write (unit=iun,fmt='(i5,(2(1x,es12.5)))') k,outthil(k),thsrc(kr)
      end do
      write(unit=iun,fmt='(a)') '------------------------------------------------'
      write(unit=iun,fmt='(a)') ' '
   end if
