...
module grell_coms
  ...
  !----- Maximum depth of inversion capping [hPa] -----------------------------------------! 
  ...
  !----- Increment on maximum depth of inversion capping [hPa] ----------------------------!
  real    , parameter  :: cap_maxs_increment = 15.
  !----------------------------------------------------------------------------------------!
  ...
  !------ MARCOS  ===== COULD YOU HELP FILL IN THE DESCRIPTION HERE 
  integer                                         :: nclouds
...
!------------------------------------------------------------------------------------------!
   subroutine define_grell_coms(ngrids,ncloud,mmzp,nnqparm,grell_1st,grell_last)

      implicit none

      integer                   , intent(in) :: ngrids     ! Number of grids (nested)
      integer                   , intent(in) :: ncloud    ! Number of clouds
      ...
      if (.not. any(grell_last == ncloud)) then
         maxens_dyn(ncloud) = 1
         maxens_eff(ncloud) = 1
         maxens_lsf(ncloud) = 1
         maxens_cap(ncloud) = 1
