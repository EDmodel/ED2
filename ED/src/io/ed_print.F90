...
! MODULE ED_PRINT
! 
!> \brief    Subroutines that print information on screen
!> \details  These subroutines print patch, cohort, site, or polygon-level data on
!>           screen.  These can be used to quickly check the simulations as they go.
!> \author   Ryan Knox
!> \author   10 Jan 2018.  Turned into module because ifort 17 complained (Marcos Longo).
!------------------------------------------------------------------------------------------!
module ed_print
   ...
   ! data to the site level and stores the data in its spatial array coordinate.  The data !
   ! is then printed to the screen, based on a specified window of data.  Note, this may   !
   ! be printing windows on various nodes, or this may be called from a master process.    !
   ! Be conscious of this; as it will dictate what part of the domain you are accessing    !
   ! variables from, and whether or not the variable of interest is stored in memory at    !
   ! that time.  For instance, many variables are stored only on the slave nodes, and need !
   ! not be passed back to the master.  Likewise, many slave node data will accumulate     !
   ! after each lsm call, until they are passed back to the master, where they are         !
   ! normalized.  These variables will be immediately zeroed on the slaves after being     !
   ! sent to the master.  Don't forget to adjust the number precision on the format string !
   ! at the end.                                                                           !
   !---------------------------------------------------------------------------------------!
   subroutine print_fields(ifm,cgrid)
      ...
      if (ipmax > cgrid%npolygons_global) then
         ...
         write(unit=*,fmt='(a,1x,i5)') ' Global number of polygons = '                     &
                                                                   ,cgrid%npolygons_global
         write(unit=*,fmt='(a)')       ' '
      ...
      !------------------------------------------------------------------------------------!
      !     Allocate the print (always) and scratch vector (if this is the "master" node   !
      ! or a serial run.                                                                   !
      !------------------------------------------------------------------------------------!
      ...
      count_pvars: do
         ...
         do nv = 1,num_var(ifm)
            ...
            if (trim(vt_info(nv,ifm)%name) .eq. trim(pvar_name)) then
               ...
               !---------------------------------------------------------------------------!
               !     If this is root, then collect the sends, keep reading to find out     !
               ...
               !---------------------------------------------------------------------------!
               !     The namelist print entry has been matched with the var_table entry.   !
               ! Now lets cycle through our machines and determine if those machines hold  !
               ! data that should be printed.  If so, then send that data to the master    !
               ! (last machine).                                                           !
               !---------------------------------------------------------------------------!
               ...
               ! index falls within the printable range. Save that pointer to a local      !
               ! array.  Once all the pointers have been cycled, send the local array to   !
               ! the master to populate a global array and print.                          !
               !---------------------------------------------------------------------------!
               ptr_send   = .false.
               node_idmin = -1
               node_idmax = -1
               do np = 1,vt_info(nv,ifm)%nptrs
                  ...
                  !------------------------------------------------------------------------!
                  !      Determine if any of this segment falls within the range desired   !
                  ! for output, globid+1 is the global index.                              !
                  !------------------------------------------------------------------------!
                  if (g_id <= ipmax .and. g_id+g_ln-1 >= ipmin ) then
                     ...
                     !---------------------------------------------------------------------!
                     !     Shift the global dataset so that it is applied to the first     !
                     ! index.  Note that only real arrays can be used this time.           !
                     !---------------------------------------------------------------------!
               ...
               if (nnodetot > 1) then
                  ...
                  if (mynum /= nnodetot) then
                     !---------------------------------------------------------------------!
                     !      Slave node.  The local array for this machine has been         !
                     ! created.  Send it off to the master.                                !
                     !---------------------------------------------------------------------!
#if defined(RAMS_MPI)

                     call MPI_Send(ptr_send,1,MPI_LOGICAL,machs(nnodetot),120              &
                                  ,MPI_COMM_WORLD,ierr)
                     ...
                     if (ptr_send) then
                        ...
                        call MPI_Send(pvar_l(node_idmin:node_idmax)                        &
                                     ,node_idmax-node_idmin+1                              &
                                     ,MPI_REAL,machs(nnodetot),123,MPI_COMM_WORLD,ierr)
                     end if
                     

                     !---------------------------------------------------------------------!
                     !     When this node is finished, send the blocking MPI_Send to the   !
                     ! next machine.                                                       !
                     !---------------------------------------------------------------------!
               ...
               !---------------------------------------------------------------------------!
               !      The data over the sought range of indices have been collected.  If   !
               ! this is the only machine or the root machine, then print it to standard   !
               ! output.                                                                   !
               !---------------------------------------------------------------------------!
         ...
         if (.not.pvartrue) then
            ...
            write (unit=*,fmt='(a)')      '  '
            write (unit=*,fmt='(a)')      ' The diagnostic variable doesn''t match any of'
            write (unit=*,fmt='(a)')      ' the var_table variables.  Check your namelist'
            write (unit=*,fmt='(a)')      ' entries, and the variable registry and/or'
            write (unit=*,fmt='(a)')      ' remove this diagnostic variable.'
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This sub-routine simply copy the variable that is stored in the main variable     !
   ! structure to the scratch array that will be exchanged.                                !
   !---------------------------------------------------------------------------------------!
