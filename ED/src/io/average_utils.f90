...
! output step (frqsum) to actual rates, and find derived properties.                       !
!------------------------------------------------------------------------------------------!
   ...
   polyloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            !------------------------------------------------------------------------------!
            !     The following variables are fluxes that cam from the RK4 and therefore   !
            ! hold the integral over time, divide them by the total time to obtain the     !
            ! mean fluxes.                                                                 !
            !------------------------------------------------------------------------------!
            ...
            !------------------------------------------------------------------------------!
            !------------------------------------------------------------------------------!
            !     Most state variables are already normalised.  All that we need to do is  !
            ! to find the derived properties.                                              !
            !------------------------------------------------------------------------------!
            ...
            ! (gross primary productivity and total plant respiration), with heterotrophic !
            ! respiration, a patch-level variable.                                         !
            !------------------------------------------------------------------------------!
   ...
   polyloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            csite%fmean_albedo_beam    (  ipa) = 0.0
            csite%fmean_albedo_diff    (  ipa) = 0.0
...
!==========================================================================================!
!==========================================================================================!
!    This subroutine normalises the daily mean variables of those variables that could not !
! be integrated directly.  This includes temperatures, polygon-level budget variables, and !
! variables that are defined during daytime only.                                          !
!------------------------------------------------------------------------------------------!
subroutine normalize_ed_dmean_vars(cgrid)
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         !---------------------------------------------------------------------------------!
         !     Find the average day length.  In case of polar night, we set the factor to  !
         ! zero so it won't become a singularity.                                          !
         !---------------------------------------------------------------------------------!
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%dmean_albedo_beam(ipa)  = csite%dmean_albedo_beam(ipa) * daylight_i
            csite%dmean_albedo_diff(ipa)  = csite%dmean_albedo_diff(ipa) * daylight_i
            !------------------------------------------------------------------------------!
            ...
            csite%dmean_can_temp(ipa) = extheta2temp(can_exner,csite%dmean_can_theta(ipa))
            ...
            !------------------------------------------------------------------------------!
            !      Aggregate the polygon-level variables of those variables that were not  !
            ! integrated during the day.                                                   !
            !------------------------------------------------------------------------------!
            ...
            cgrid%dmean_albedo_beam    (ipy) = cgrid%dmean_albedo_beam    (ipy)            &
                                             ...
                                             * patch_wgt
            cgrid%dmean_albedo_diff    (ipy) = cgrid%dmean_albedo_diff    (ipy)            &
                                             + csite%dmean_albedo_diff    (ipa)            &
                                             * patch_wgt
            ...
            cohortloop: do ico=1,cpatch%ncohorts
               ...
               !---------------------------------------------------------------------------!
               !      Aggregate the polygon-level variables of the variables that have not !
               ! been aggregated during the previous day.                                  !
               !---------------------------------------------------------------------------!
      ...
      !------------------------------------------------------------------------------------!
      do k=1,nzg
         call uextcm2tl(cgrid%dmean_soil_energy(k,ipy),cgrid%dmean_soil_water(k,ipy)*wdns  &
                       ,cgrid_dmean_soil_hcap  (k)    ,cgrid%dmean_soil_temp (k,ipy)       &
   ...
   polyloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%dmean_albedo_beam      (ipa) = 0.0
            csite%dmean_albedo_diff      (ipa) = 0.0
...
!==========================================================================================!
!==========================================================================================!
!    This subroutine integrates most of the monthly mean variables.  This sub-routine is   !
! called after the dmean variables have been normalised, so we can take advantage of their !
! values.                                                                                  !
!    A few variables are _NOT_ integrated here: quantities such as temperature and liquid  !
! fraction are found after the monthly mean of the extensive quantities have been          !
! normalised. Also, polygon-level variables that were not integrated to the daily means    !
! are not integrated here, they are found after the monthly mean at the native level has   !
! been found.                                                                              !
!------------------------------------------------------------------------------------------!
subroutine integrate_ed_mmean_vars(cgrid)
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      !------------------------------------------------------------------------------------!
      !    The following variables don't have daily means, but the instantaneous value is  !
      ! fine because they are updated only once a day.                                     !
      !------------------------------------------------------------------------------------!
      ...
      cgrid%mmean_albedo_beam      (ipy) = cgrid%mmean_albedo_beam      (ipy)              &
                                         ...
                                         * ndaysi
      cgrid%mmean_albedo_diff      (ipy) = cgrid%mmean_albedo_diff      (ipy)              &
                                         + cgrid%dmean_albedo_diff      (ipy)              &
                                         * ndaysi
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%mmean_albedo_beam      (ipa) = csite%mmean_albedo_beam      (ipa)        &
                                               + csite%dmean_albedo_beam      (ipa)        &
                                               * ndaysi
            csite%mmean_albedo_diff      (ipa) = csite%mmean_albedo_diff      (ipa)        &
                                               + csite%dmean_albedo_diff      (ipa)        &
                                               * ndaysi
...
!==========================================================================================!
!    This subroutine normalises the daily mean variables of those variables that could not !
! be integrated directly.  This includes temperatures, liquid fraction, and soil matric    !
! potential.                                                                               !
!------------------------------------------------------------------------------------------!
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%mmean_can_temp(ipa) = extheta2temp(can_exner,csite%mmean_can_theta(ipa))
      ...
      !------------------------------------------------------------------------------------!
      do k=1,nzg
         call uextcm2tl(cgrid%mmean_soil_energy(k,ipy),cgrid%mmean_soil_water(k,ipy)*wdns  &
                       ,cgrid_mmean_soil_hcap  (k)    ,cgrid%mmean_soil_temp (k,ipy)       &
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%mmean_albedo_beam      (ipa) = 0.0
            csite%mmean_albedo_diff      (ipa) = 0.0
...
! called after the fmean variables have been normalised, so we can take advantage of their !
! values.                                                                                  !
!                                                                                          !
!    A few variables are _NOT_ integrated here: quantities such as temperature and liquid  !
! fraction are found after the mean diel of the extensive variables have been normalised.  !
!------------------------------------------------------------------------------------------!
subroutine integrate_ed_qmean_vars(cgrid)
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      cgrid%qmean_albedo_beam      (t,ipy) = cgrid%qmean_albedo_beam      (t,ipy)          &
                                           + cgrid%fmean_albedo_beam        (ipy)          &
                                           * ndaysi
      cgrid%qmean_albedo_diff      (t,ipy) = cgrid%qmean_albedo_diff      (t,ipy)          &
                                           + cgrid%fmean_albedo_diff        (ipy)          &
                                           * ndaysi
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%qmean_albedo_beam      (t,ipa) = csite%qmean_albedo_beam      (t,ipa)    &
                                                 ...
                                                 * ndaysi
            csite%qmean_albedo_diff      (t,ipa) = csite%qmean_albedo_diff      (t,ipa)    &
...
!==========================================================================================!
!    This subroutine normalises the daily mean variables of those variables that could not !
! be integrated directly.  These are pretty much just temperature, density, and liquid     !
! water fraction.                                                                          !
!------------------------------------------------------------------------------------------!
   ...
   polyloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi=1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%qmean_albedo_diff            (:,ipa) = 0.0
            ...
            cohortloop: do ico = 1,cpatch%ncohorts
               ...
               if (cpatch%new_recruit_flag(ico) == 1) then
                  cgrid%total_agb_recruit       (ipy)= cgrid%total_agb_recruit      (ipy)  &
                                                     ...
                                                     * cpatch%nplant                (ico)  &
                                                     * patch_wgt
                  cgrid%total_basal_area_recruit(ipy)= cgrid%total_basal_area_recruit(ipy) &
                                                     + cpatch%basarea(ico)                 &
                                                     * cpatch%nplant (ico)                 &
                                                     * patch_wgt
