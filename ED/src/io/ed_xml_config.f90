...
! photosynthesis variables
           ...
           if(texist) curvpar_electron(myPFT) = real(rval)
           call getConfigREAL  ('qyield_psII','pft',i,rval,texist)
           if(texist) qyield_psII(myPFT) = real(rval)
           ...
           if (texist) then
              Vm_decay_elow (myPFT) = real(rval)
              Vm_decay_ehigh(myPFT) = real(rval)
           end if
           call getConfigREAL  ('Vm_decay_elow','pft',i,rval,texist)
           if(texist) Vm_decay_elow(myPFT) = real(rval)
           call getConfigREAL  ('Vm_decay_ehigh','pft',i,rval,texist)
           if(texist) Vm_decay_ehigh(myPFT) = real(rval)
...
!! Leaf Respiration
           ...
           if (texist) then
              Rd_decay_elow (myPFT) = real(rval)
              Rd_decay_ehigh(myPFT) = real(rval)
           end if
           call getConfigREAL  ('Rd_decay_elow','pft',i,rval,texist)
           if(texist) Rd_decay_elow(myPFT) = real(rval)
           call getConfigREAL  ('Rd_decay_ehigh','pft',i,rval,texist)
           if(texist) Rd_decay_ehigh(myPFT) = real(rval)
...
! respiration & turnover variables
           ...
           if (texist) then
              rrf_decay_elow (myPFT) = real(rval)
              rrf_decay_ehigh(myPFT) = real(rval)
           end if
           call getConfigREAL  ('rrf_decay_elow','pft',i,rval,texist)
           if(texist) rrf_decay_elow(myPFT) = real(rval)
           call getConfigREAL  ('rrf_decay_ehigh','pft',i,rval,texist)
           if(texist) rrf_decay_ehigh(myPFT) = real(rval)
