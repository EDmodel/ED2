module lapse
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This subroutine calculates the site-level variables based on the polygon-level    !
   ! ones.  If needed, it will also apply the lapse rate on some variables, and            !
   ! recalculate the others in order to keep satisfying the ideal gas law and some thermo- !
   ! dynamic properties.                                                                   !
   !---------------------------------------------------------------------------------------!
   subroutine calc_met_lapse(cgrid,ipy)
      ...
      if (bypass) then
         ...
         do isi = 1,cpoly%nsites
            ...
            cpoly%met(isi)%geoht       = cgrid%met(ipy)%geoht                              &
                                       + cgrid%lapse(ipy)%geoht   * delE
            cpoly%met(isi)%atm_tmp     = cgrid%met(ipy)%atm_tmp                            &
                                       + cgrid%lapse(ipy)%atm_tmp * delE
            cpoly%met(isi)%atm_shv     = max(toodry, cgrid%met(ipy)%atm_shv                &
                                       + cgrid%lapse(ipy)%atm_shv * delE)
            cpoly%met(isi)%prss        = cgrid%met(ipy)%prss                               &
                                       + cgrid%lapse(ipy)%prss    * delE
            cpoly%met(isi)%atm_co2     = cgrid%met(ipy)%atm_co2                            &
                                       + cgrid%lapse(ipy)%atm_co2 * delE
            cpoly%met(isi)%rlong       = cgrid%met(ipy)%rlong                              &
                                       + cgrid%lapse(ipy)%rlong   * delE
            ...
            !------------------------------------------------------------------------------!
            ! Note: at this point VELS is vel^2.  Thus this lapse preserves mean kinetic   !
            ! ENERGY not wind SPEED.                                                       !
            !------------------------------------------------------------------------------!
            cpoly%met(isi)%vels      = cgrid%met(ipy)%vels                                 &
