...
module therm_lib
   ...
   real function calc_hcapveg(leaf_biomass,structural_biomass,nplants,pft)
     ...
     real :: spec_hcap_leaf
     real :: spec_hcap_stem
     real,parameter :: min_hcapveg = 300.0  ! 300 is roughly 1.5 kg's of biomass at 3000 J/kg/K
     integer :: pft     
     ...
     !calc_hcapveg = 3000*leaf_biomass*nplants
...
!     print*,"cweh",cpatch%veg_energy(ico),cpatch%veg_temp(ico),cpatch%hcapveg(ico),cpatch%veg_water(ico)
...
!     print*,"ct",cpatch%veg_energy(ico),cpatch%veg_temp(ico),cpatch%hcapveg(ico),cpatch%veg_water(ico)

     if(cpatch%veg_temp(ico) < 183.15) then
        print*,"unusual veg temp in update_veg_energy_ct",cpatch%veg_temp(ico)
     endif
     return
