...
module ed_therm_lib
   ...
   subroutine hmv2tqz(enthalpy,mvap,prss,rhos,temp,shv,depth,reject_step)
      ...
      implicit none
      !----- Arguments --------------------------------------------------------------------!     
      real   , intent(in)    :: enthalpy    ! Enthalpy                          [     J/m淫
      real   , intent(in)    :: mvap        ! Water vapour mass                 [    kg/m淫
      real   , intent(in)    :: prss        ! Pressure                          [     N/m淫
      ...
      real              :: ac4              ! 4*A*C of Baskara quadratic eqn.   [       K淫
      !------------------------------------------------------------------------------------!
   ...
   subroutine hmv2tqz8(enthalpy,mvap,prss,rhos,temp,shv,depth,reject_step)
      ...
      implicit none
      !----- Arguments --------------------------------------------------------------------!     
      real(kind=8), intent(in)    :: enthalpy    ! Enthalpy                     [     J/m淫
      real(kind=8), intent(in)    :: mvap        ! Water vapour mass            [    kg/m淫
      real(kind=8), intent(in)    :: prss        ! Pressure                     [     N/m淫
      real(kind=8), intent(in)    :: rhos        ! Air density                  [    kg/m設
      real(kind=8), intent(inout) :: temp        ! Temperature                  [        K]
      real(kind=8), intent(inout) :: shv         ! Specific humidity            [kg_vap/kg]
      real(kind=8), intent(inout) :: depth       ! Depth of the reservoir       [        m]
      logical     , intent(inout) :: reject_step ! Should this step be rejected?[      T|F]
      !----- Local variables --------------------------------------------------------------!
      real(kind=8)              :: b             ! "B" term of Baskara eqn .    [        K]
      real(kind=8)              :: ac4           ! 4*A*C of Baskara eqn.        [       K淫
      real(kind=8)              :: min_enthalpy  ! Minimum acceptable enthalpy  [     J/m淫
      real(kind=8)              :: min_mvap      ! Minimum acceptable mass      [    kg/m淫
      real(kind=8)              :: max_enthalpy  ! Maximum acceptable enthalpy  [     J/m淫
      real(kind=8)              :: max_mvap      ! Maximum acceptable mass      [    kg/m淫
      !------------------------------------------------------------------------------------!
      ...
      !------------------------------------------------------------------------------------!
      !      Finding the auxiliary terms used to find temperature as a function of water   !
      ! vapour mass and enthalpy.  Since this can be called by a bad time step, we must    !
