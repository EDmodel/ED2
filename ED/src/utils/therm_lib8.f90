...
module therm_lib8
   ...
   real(kind=8) function idealdmolsh8(pres,temp,qvpr)
      use consts_coms, only : rmol8 & ! intent(in)
                            , ep8   ! ! intent(in)
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      real(kind=8), intent(in)           :: pres ! Pressure                        [    Pa]
   ...
   !---------------------------------------------------------------------------------------!
   real(kind=8) function dthetail_dt8(condconst,thil,pres,temp,rliq,ricein)
      use consts_coms, only : alvl38      & ! intent(in)
                            , alvi38      & ! intent(in)
                            , dcpvi8      & ! intent(in)
                            , dcpvl8      & ! intent(in)
   ...
   real(kind=8) function thil2temp8(thil,exner,pres,rliq,rice,t1stguess)
      ...
      deriv     = dthetail_dt8(.true.,fun,pres,tempz,rliq,rice)
      fun       = fun - thil
      !------------------------------------------------------------------------------------!
   ...
   subroutine thil2tqall8(thil,exner,pres,rtot,rliq,rice,temp,rvap,rsat)
      ...
      deriv  = dthetail_dt8(.false.,funnow,pres,tempz,rliq,rice)
      funnow = funnow - thil
      fun1st = funnow
      !------------------------------------------------------------------------------------!
   ...
   subroutine thil2tqliq8(thil,exner,pres,rtot,rliq,temp,rvap,rsat)
      ...
      deriv  = dthetail_dt8(.false.,funnow,pres,tempz,rliq)
