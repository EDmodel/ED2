...
!==========================================================================================!
!==========================================================================================!
!     This function sorts a vector.  This is a very inefficient algorithm.  For a better   !
! one use heapsort.                                                                        !
!------------------------------------------------------------------------------------------!
subroutine fsort(nx,x,increase,xsort)
   implicit none
   !----- Arguments. ----------------------------------------------------------------------!
   integer               , intent(in)  :: nx       ! Size of input/output vectors
   real   , dimension(nx), intent(in)  :: x        ! Input vector
   logical               , intent(in)  :: increase ! Sort from small to large?
   real   , dimension(nx), intent(out) :: xsort    ! Output vector
   !----- Local variables. ----------------------------------------------------------------!
   logical, dimension(nx)              :: avail    ! Mask to exclude selected data
   integer                             :: n        ! Counter
   integer                             :: isel     ! Index of selected data
   !---------------------------------------------------------------------------------------!
   ...
   !---------------------------------------------------------------------------------------!
   !     Decide whethet to go from lowest to highest or highest to lowest.                 !
   !---------------------------------------------------------------------------------------!
   if (increase) then
      !----- Lowest to highest. -----------------------------------------------------------!
      do n=1,nx
         !----- Find the lowest selectable number. ----------------------------------------!
         isel = minloc(x,dim=1,mask=avail)
         !---------------------------------------------------------------------------------!

         !----- Copy lowest selectable value to current output position. ------------------!
         xsort(n) = x(isel)
         !---------------------------------------------------------------------------------!

         !------ Block the input vector element we have just selected. --------------------!
         avail(isel) = .false.
         !---------------------------------------------------------------------------------!
      end do
      !------------------------------------------------------------------------------------!
      ...
      !----- Highest to lowest. -----------------------------------------------------------!
      do n=1,nx
         !----- Find the highest selectable number. ---------------------------------------!
         isel = maxloc(x,dim=1,mask=avail)
         !---------------------------------------------------------------------------------!

         !----- Copy highest selectable value to current output position. -----------------!
         xsort(n) = x(isel)
         !---------------------------------------------------------------------------------!
