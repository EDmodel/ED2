...
module phenology_aux
   ...
   subroutine update_turnover(cpoly, isi)
      ...
      real                                 :: sla_now
      real                                 :: vm0_now
      real              , dimension(n_pft) :: vm_bar_toc_in
      real              , dimension(n_pft) :: llspan_toc_in
      real              , dimension(n_pft) :: sla_toc_in
      !------------------------------------------------------------------------------------!



      !------ Copy original canopy traits, to rescale cohort properties. ------------------!
      vm_bar_toc_in(:) = cpoly%vm_bar_toc(:,isi)
      llspan_toc_in(:) = cpoly%llspan_toc(:,isi)
      sla_toc_in   (:) = cpoly%sla_toc   (:,isi)
      !------------------------------------------------------------------------------------!
      ...
      case (1)
         !---------------------------------------------------------------------------------!
         !      Parametrisation based on long-term measurements of leaf litter fall.       !
         ! From observations, a log-linear approach seems more appropriate than linear     !
         !---------------------------------------------------------------------------------!
         ...
         elseif (cpoly%rad_avg(isi) >= radto_max) then
            ...
            turnamp_now = radint * cpoly%rad_avg(isi) ** radslp
         end if
         !---------------------------------------------------------------------------------!
      case default
         !----- Original parametrisation (Kim et al. (2012). ------------------------------!
         if (cpoly%rad_avg(isi) <= radto_min) then
            turnamp_now = turnamp_min
         elseif (cpoly%rad_avg(isi) >= radto_max) then
            turnamp_now = turnamp_max
      ...
      !------------------------------------------------------------------------------------!
      !      Loop through all the PFTs and update the reference values.                    !
      !------------------------------------------------------------------------------------!
      sitepft_loop: do ipft=1,n_pft
         ...
         case (3)
            ...
            if (leaf_turnover_rate(ipft) > 0.) then
               llspan_now = 12.0 / ( cpoly%turnover_amp(isi) * leaf_turnover_rate(ipft) )
            ...
            !------------------------------------------------------------------------------!
            cpoly%llspan_toc(ipft,isi) = (1.0 - llspan_wgt) * cpoly%llspan_toc(ipft,isi)   &
                                       +        llspan_wgt  * llspan_now
            !------------------------------------------------------------------------------!


            !------------------------------------------------------------------------------!
            !     For Vm0 and SLA, we must do things differently depending on the          !
            ! economics scheme.                                                            !
            !------------------------------------------------------------------------------!
            select case (economics_scheme)
            case (1)
               !---------------------------------------------------------------------------!
               !    Use the trait relationships from L20 for the "now" terms.              !
               !                                                                           !
               ...
               !---------------------------------------------------------------------------!
               sla_now = sla_s0(ipft) * ( 12.0 / llspan_now ) ** sla_s1(ipft)
               vm0_now = vm0_v0(ipft) * sla_now               ** vm0_v1(ipft)
               !---------------------------------------------------------------------------!


               !---------------------------------------------------------------------------!
               !       Apply running averages on both SLA and Vm0.  Note that the weights  !
               ! are not swapped in SLA: the running average is being applied on LMA       !
               ! instead.                                                                  !
               !---------------------------------------------------------------------------!
               cpoly%sla_toc   (ipft,isi) = cpoly%sla_toc(ipft,isi) * sla_now              &
                                          / (        sla_wgt  * cpoly%sla_toc (ipft,isi)   &
                                            + (1.0 - sla_wgt) * sla_now                  )
               cpoly%vm_bar_toc(ipft,isi) = (1.0 - vm0_wgt) * cpoly%vm_bar_toc(ipft,isi)   &
                                          +        vm0_wgt  * vm0_now
               !---------------------------------------------------------------------------!
            case default
               !---------------------------------------------------------------------------!
               !    Original approach, from K12.  We apply running average on Vm_bar, but  !
               ! not on SLA, which is corrected through the turnover amplitude (itself     !
               ! corrected by moving average).                                             !
               !                                                                           !
               ...
               vm0_now                    = vm0_min                                        &
                                          + vm0_amp                                        &
                                          / ( 1. +  (cpoly%llspan_toc(ipft,isi)/vm0_tran)  &
                                                 ** vm0_slope                             )
               cpoly%vm_bar_toc(ipft,isi) = (1.0 - vm0_wgt) * cpoly%vm_bar_toc(ipft,isi)   &
                                          +        vm0_wgt  * vm0_now
               !---------------------------------------------------------------------------!
               ...
               turnover_now            = cpoly%turnover_amp(isi) * leaf_turnover_rate(ipft)
               cpoly%sla_toc(ipft,isi) = sla_s0(ipft) * turnover_now ** sla_s1(ipft)
               !---------------------------------------------------------------------------!
            end select
            !------------------------------------------------------------------------------!
         case default
            !----- The default is to not change anything. ---------------------------------!
            continue
            !------------------------------------------------------------------------------!
         end select
         !---------------------------------------------------------------------------------!
      ...
      patchloop: do ipa = 1,csite%npatches
         ...
         !---------------------------------------------------------------------------------!
         !       Update cohort traits based on the updated canopy reference.               !
         !---------------------------------------------------------------------------------!
         ...
         cohortloop: do ico = 1,cpatch%ncohorts
            ...
            !------------------------------------------------------------------------------!
            !     Check phenology: it's not necessary to update PFTs that are not light-   !
            ! -modulated.                                                                  !
            !------------------------------------------------------------------------------!
            select case (phenology(ipft))
            case (3)
               ...
               cpatch%vm_bar(ico) = cpatch%vm_bar   (ico)                                  &
                                  * cpoly%vm_bar_toc(ipft,isi) / vm_bar_toc_in(ipft)
               cpatch%sla   (ico) = cpatch%sla      (ico)                                  &
                                  * cpoly%sla_toc   (ipft,isi) / sla_toc_in   (ipft)
               !---------------------------------------------------------------------------!
            case default
