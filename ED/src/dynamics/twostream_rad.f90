!==========================================================================================!
!==========================================================================================!
! MODULE TWOSTREAM_RAD                                                                     !
!                                                                                          !
!> \brief  Sub-routines that solve the two-stream model for SW and LW radiation            !
!> \details These sub-routines solve the multi-layer two stream model to obtain the        !
!>          net absorption of irradiance by each cohort.  These routines are similar to    !
!>          and intended to replace them eventually.                                       !
!> \author  Feb 2013 First development, based on Liou's book (Marcos Longo)                !
!> \author  21 Oct 2017 Turned the file into a module for safer compilation (Marcos Longo) !
!------------------------------------------------------------------------------------------!
module twostream_rad
   contains
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This sub-routine solves the long-wave radiation method using the two-stream       !
   ! model.  We consider the finite crown area when computing the layer transmittance.     !
   ...
   !                                                                                       !
   !---------------------------------------------------------------------------------------!
   subroutine lw_two_stream(grnd_emiss4,grnd_temp4,rlong_top4,ncoh,pft,lai,wai,cai         &
      ...
      do i=1,ncoh
         ...
         !---------------------------------------------------------------------------------!
         !      Black is the emission of a black body at the same temperature of the       !
         ! layer.  The temperature is the weighted average where the weights are the       !
         ! product between the area and the emissivity.                                    !
         !---------------------------------------------------------------------------------!
         ...
         !---------------------------------------------------------------------------------!
         !      iota is the single scattering albedo.  Because we assume absorptivity to   !
         ! be the same as emissivity, we use the latter to define it.                      !
         !---------------------------------------------------------------------------------!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This sub-routine solves the short-wave radiation method using the two-stream      !
   ! model.  We consider the finite crown area when computing the transmittance of both    !
   ...
   subroutine sw_two_stream(grnd_alb_par4,grnd_alb_nir4,cosaoi4,nir_beam_norm              &
                           ...
                           ,light_level,light_beam_level,light_diff_level)
      use ed_max_dims          , only : n_pft                   & ! intent(in)
                                      , n_radprof               ! ! intent(in)
      use rk4_coms             , only : tiny_offset             ! ! intent(in)
      use canopy_radiation_coms, only : clumping_factor         & ! intent(in)
   ...
   !   par_level_diffu(:) = 0.d0
      ...
      !------------------------------------------------------------------------------------!
      !     Here we loop over the two bands (band 1 is PAR and band 2 is NIR), and solve   !
      ...
      bandloop: do iband = 1,2
         ...
         directloop: do i=1,ncoh
            ...
            !------------------------------------------------------------------------------!
            !     We find the inverse optical depth of the direct radiation (mu0), follow- !
            ! ing CLM10 (equation 3.3 and text after equation 3.3).                        !
            !------------------------------------------------------------------------------!
            ...
            !------------------------------------------------------------------------------!
            !     The inverse optical depth of the diffuse radiation (mu), same as in      !
            ! longwave radiation.                                                          !
            !------------------------------------------------------------------------------!
            mu (i) = - etai(i) / log( ( 1.d0 - cai(i) )                                    &
            ...
            !------------------------------------------------------------------------------!
            !      Find the backscatter for direct beam radiation, following CLM10,        !
            ! equations (3.14) and (3.15).  The forward scattering was omitted on both     !
            ! equations because they are different for PAR and NIR, but in both cases they !
            ! would cancel out.  epsil0 = 1 - 2*beta0, no special definition...            !
            !------------------------------------------------------------------------------!
         ...
         do i=1,ncoh+1
            ...
            !------------------------------------------------------------------------------!
            !      Retrieve the downward and upward diffuse (hemispheric) radiation, by    !
            ! using the solutions for full TAI                                             !
            !------------------------------------------------------------------------------!
            down(i) = xvec(i2m1) * gamm_plus (i) * expl_minus (i)                          &
                    + xvec  (i2) * gamm_minus(i) * expl_plus  (i)                          &
         ...
         case (1)
            ...
            ! These are fluxes, not absorbed, as a fraction of total PAR.  The current     !
            ! units of up and down are fraction of total rshort, so we have to divide by   !
            ! the total PAR fraction of rshort, the units are fraction of total PAR        !
            !------------------------------------------------------------------------------!

            do i=1,ncoh
               ip1 = i + 1
               par_level_diffu(i) = 5.d-1*(up(i)    + up(ip1))                             &
                                  / (par_beam_norm+par_diff_norm)
               par_level_diffd(i) = 5.d-1*(down(i)  + down(ip1))                           &
                                  / (par_beam_norm+par_diff_norm)
               par_level_beam(i)  = 5.d-1*(down0(i) + down0(ip1))                          &
                                  / (par_beam_norm+par_diff_norm)
            end do
            !------------------------------------------------------------------------------!
