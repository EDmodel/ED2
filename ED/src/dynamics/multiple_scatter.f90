module multiple_scatter
   contains

   !=======================================================================================!
   !=======================================================================================!
   !     This sub-routine solves the long-wave radiation method using the multiple-        !
   ! -scattering model.  We consider the finite crown area when computing the layer        !
   ! transmittance.  Contrary to short-wave radiation, we don't normalise long-wave        !
   ! radiation.                                                                            !
   !                                                                                       !
   ! References:                                                                           !
   !                                                                                       !
   ...
   !                                                                                       !
   ! Oleson, K. W., and co-authors, 2004: Technical description of the community land      !
   !    model(CLM). NCAR Technical note NCAR/TN-461+STR. 186pp. (CLM04)                    !
   !                                                                                       !
   ...
   subroutine lw_multiple_scatter(grnd_emiss4,grnd_temp4,rlong_top4,ncoh,pft,lai,wai,cai   &
      ...
      ! properties of leaves/branches (i.e., properties that do not depend on which band   !
      ! we are solving):  the extinction coefficient lambda, the layer transmittance       !
      ...
      do i=1,ncoh
         ...
         ! similar way from equation (2) of ZQ06.  We must integrate the contribution      !
         ! coming from each hemispheric direction.  Again, we must consider the finite     !
         ! crown area, and use the same extinction function we use for shortwave           !
         ! radiation, which comes  from CLM04 equation 3.3.                                !
         ...
         !---------------------------------------------------------------------------------!
         !      Thermal emission.  The temperature is the weighted average where the       !
         ! weights are the product between the area and the emissivity.                    !
         !---------------------------------------------------------------------------------!
      ...
      !------------------------------------------------------------------------------------!
      !     Define some boundary conditions for transmission, emission, and back-          !
      ! scattering, and the complement of the three properties.  The boundary conditions   !
      ! come from ZQ06, equations 16-22.                                                   !
      !------------------------------------------------------------------------------------!
      tau       (0) = 0.d0
      tau  (ncoh+1) = 1.d0
      r         (0) = 1.d0
      ...
      ! based on equations (8) and (9) of ZQ06, with the difference that we find LWd_i     !
      ! instead of LWd_i+1.                                                                !
      !------------------------------------------------------------------------------------!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This sub-routine solves the short-wave radiation method using the multiple-       !
   ! -scattering model.  We consider the finite crown area when computing the              !
   ! transmittance of both direct and diffuse radiation.  References:                      !
   !                                                                                       !
   ...
   !    Resources Res., 41, W08409, doi: 10.1029/2005WR004016. (ZQ05)                      !
   !                                                                                       !
   ! Oleson, K. W., and co-authors, 2004: Technical description of the community land      !
   !   model (CLM). NCAR Technical note NCAR/TN-461+STR. 186pp. (CLM04)                    !
   !                                                                                       !
   ...
   subroutine sw_multiple_scatter(grnd_alb_par4,grnd_alb_nir4,cosaoi4,nir_beam_norm        &
                                 ...
                                 ,par_level_diffu,light_level,light_beam_level             &
                                 ,light_diff_level)
      use ed_max_dims          , only : n_pft                   & ! intent(in)
                                      , n_radprof               ! ! intent(in)
      use rk4_coms             , only : tiny_offset             ! ! intent(in)
      ...
      !------------------------------------------------------------------------------------!
      !     Here we loop over the two bands (band 1 is PAR and band 2 is NIR), following   !
      ! the algorithm proposed by ZQ05.                                                    !
      !------------------------------------------------------------------------------------!
      bandloop: do iband = 1,2
         ...
         ! function.  We define the beam_down vector in a slightly different way from      !
         ! ZQ05, because here we will define S_i as the radiation that reaches the bottom  !
         ! of the layer, whereas they defined it as the radiation that reaches the top.    !
         !---------------------------------------------------------------------------------!
         do i=ncoh,1,-1
         ...
         !---------------------------------------------------------------------------------!
         !     Copy the solution without considering the multiple scattering to the        !
         ! vectors.                                                                        !
         !---------------------------------------------------------------------------------!
         swu0(0)      = swvec(1)
         ...
         !---------------------------------------------------------------------------------!
         !     Find the diffuse radiation when multiple scattering is considered.  This is !
         ! based on equations (24) and (25) of ZQ05, with the difference that we find      !
         ! SWd_i instead of SWd_i+1.                                                       !
         !---------------------------------------------------------------------------------!
         !----- Downward diffuse (hemispheric) radiation. ---------------------------------!
         do i=1,ncoh+1
            im1 = i - 1
         ...
         do i=1,ncoh
            ...
            light_level     (i) = light_level     (i)                                      &
                                + 5.d-1 * ( swd(i) + swd(ip1)                              &
                                          + beam_down(i) + beam_down(ip1))
            light_beam_level(i) = light_beam_level(i)                                      &
                                + 5.d-1 * (beam_down(i) + beam_down(ip1))
            light_diff_level(i) = light_diff_level(i)                                      &
                                + 5.d-1 * (swd(i) + swd(ip1))
         end do
         !---------------------------------------------------------------------------------!
         ...
         case (1)
            ...
            do i=1,ncoh
               im1 = i - 1
               ip1 = i + 1
               par_beam_flip  (i) = sngloff( beam_down(ip1) - beam_down(i)                 &
                                           , tiny_offset )
               par_diff_flip  (i) = sngloff( swd(ip1) - swd(i) + swu(im1) - swu(i)         &
                                           , tiny_offset )
         ...
         case (2)
            ...
            do i=1,ncoh
               im1 = i - 1
               ip1 = i + 1
               nir_beam_flip  (i) = sngloff( beam_down(ip1) - beam_down(i)                 &
                                           , tiny_offset )
               nir_diff_flip  (i) = sngloff(swd(ip1) - swd(i) + swu(im1) - swu(i)          &
                                           , tiny_offset )
