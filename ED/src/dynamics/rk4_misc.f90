...
subroutine print_csiteipa(csite, ipa)
   ...
   write (unit=*,fmt='(8(a12,1x))')  '   DIST_TYPE','         AGE','        AREA'          &
                                    ,'     RH(nzg)','      CWD_RH','AVGDAILY_TMP'          &
                                    ,'     SUM_CHD','     SUM_DGD'
   write (unit=*,fmt='(i12,1x,7(es12.4,1x))')  csite%dist_type(ipa),csite%age(ipa)         &
         ,csite%area(ipa),csite%rh(nzg,ipa),csite%cwd_rh(ipa),csite%avg_daily_temp(ipa)    &
...
subroutine print_rk4patch(y,csite,ipa)
   ...
   write (unit=*,fmt='(2(es12.4,1x))') y%rh(nzg),y%cwd_rh
...
subroutine print_rk4_state(initp,fluxp,csite,ipa,isi,elapsed,hdid)
   ...
   sum_lai         = 0.d0
   sum_wai         = 0.d0
   soil_rh         = initp%rh(nzg)-initp%cwd_rh
   cpatch => csite%patch(ipa)
   do ico=1,cpatch%ncohorts
      if (initp%leaf_resolvable(ico)) then
