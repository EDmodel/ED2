module old_twostream_rad
   contains
   !=======================================================================================!
   !=======================================================================================!
   !    This subroutine will solve the within canopy two-stream radiation for shortwave,   !
   ! considering the light absorption by leaves, and branches if the user wants so.  In    !
   ! both cases we consider the clumpiness effect (i.e., the fact that leaves and branches !
   ...
   subroutine old_sw_two_stream (salbedo_par,salbedo_nir,scosaoi,nir_beam_norm             &
      ...
      do il=1,ncoh
         ...
         !---------------------------------------------------------------------------------!
         !     We find the optical depth of the direct beam (lambda), following CLM        !
         ! technical note (equation 3.3 and text after equation 3.2).                      !
         !---------------------------------------------------------------------------------!
         ...
         ! and (3.15) of the CLM technical note.  The forward scattering was omitted on    !
         ! both equations because they are different for PAR and NIR, but in both cases    !
         ! they cancel out.                                                                !
         !---------------------------------------------------------------------------------!
      ...
      bandloop: do iband = 1,2
         ...
         !---------------------------------------------------------------------------------!
         !     Calculate more factors for this band.  We start by calculating the          !
         ! forcings, using the effective tree area index, considering the leaf clumpiness  !
         ! and, if the user asked for it, the branchwood area index (in case the user      !
         ! doesn't want, WAI=0.).  We distinguish between the light underneath a crown     !
         ! (beam_bot_crown) and the total light passing through a layer (beam_bot).  Also, !
         ! we consider that the  canopy may be partially open in case the user wants this  !
         ! distinction (otherwise canopy_area is one.).                                    !
         !---------------------------------------------------------------------------------!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This subroutine will solve the long wave radiation within the canopy.  This       !
   ! solves the two stream approach considering the size distribution of cohorts, acknow-  !
   ! ledging the effect of leaves, and, if that's the user's will, the branches.           !
   !---------------------------------------------------------------------------------------!
   subroutine old_lw_two_stream(semgs,sT_grnd,rlong_top4,ncoh, pft,lai,wai,leaf_temp       &
      ...
      real                                   , intent(in)    :: semgs      ! Gnd. emmiss.
      real                                   , intent(in)    :: st_grnd    ! Gnd. temp.
      real(kind=4)                           , intent(in)    :: rlong_top4
      integer     , dimension(ncoh)          , intent(in)    :: pft        ! Pl. func. type
      real(kind=8), dimension(ncoh)          , intent(in)    :: lai        ! Leaf Area Idx
      real(kind=8), dimension(ncoh)          , intent(in)    :: wai        ! Wood Area Idx
      real(kind=8), dimension(ncoh)          , intent(in)    :: leaf_temp  ! Leaf temp.
      real(kind=8), dimension(ncoh)          , intent(in)    :: wood_temp  ! Wood temp.
      ...
      do il=1,ncoh
         ...
         !---------------------------------------------------------------------------------!
         !     Coefficient of lambda1 (and minus the coefficient of lambda2) for the       !
         ! bottom of a layer, downwelling radiation.                                       !
         !---------------------------------------------------------------------------------!
         a_dw(il) = 5.d-1 * exki
      ...
      downward_lw_surf(1)       = a_dw(1)                                                  &
                                * (mastervec_surf(ncoh2-1)  - mastervec_surf(ncoh2))
      downward_lw_incid(1)      = a_dw(1)                                                  &
                                * (mastervec_incid(ncoh2-1) - mastervec_incid(ncoh2))
