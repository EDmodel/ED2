!==========================================================================================!
!==========================================================================================!
!    This subroutine initialises derived radiation parameters.  Instead of calling these   !
! numbers every time step, we define these values only once, during the initialisation     !
! step.                                                                                    !
!------------------------------------------------------------------------------------------!
subroutine init_derived_rad_params()
   use canopy_radiation_coms, only : leaf_trans_vis       & ! intent(in)
   ...
   return
end subroutine init_derived_rad_params
!==========================================================================================!
!==========================================================================================!
   ...
   if (mod(current_time%time + .001,radfrq) < dtlsm) then
      ...
      polyloop: do ipy = 1,cgrid%npolygons
         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            !------------------------------------------------------------------------------!
            !    MLO. Changed this part of the code so it also works with the horizontal   !
            ! shading.  Essentially I took the loop through patches out of sfcrad_ed and   !
            ! scale_ed_radiation, so each patch could use different normalisation factors. !
            !------------------------------------------------------------------------------!
            patchloop: do ipa=1,csite%npatches
               ...
               ! close to zero), eliminate all direct radiation.   This is different from  !
               ! the cosine of the zenith angle because mountains can hide the sun even    !
               ! when it is still above the horizon.                                       !
               !---------------------------------------------------------------------------!
               if (daytime) then
                  !------------------------------------------------------------------------!
...
subroutine sfcrad_ed(cosaoi,csite,ipa,mzg,mzs,ntext_soil,ncol_soil,tuco,rlong,twilight)
   ...
   !---------------------------------------------------------------------------------------!
   !     Decide what is our surface temperature.  When the soil is exposed, then that is   !
   ! the surface temperature.  Otherwise, we pick the temporary surface water or snow      !
   ! layer.                                                                                !
   !---------------------------------------------------------------------------------------!
   rad_sfcw_par      = 1.0
   rad_sfcw_nir      = 1.0
   ...
   if (ksn == 0) then
      ...
      !------------------------------------------------------------------------------------!
      !      Sfcwater albedo ALS ranges from wet-soil value for all-liquid to typical snow !
      ! albedo for ice.  In the future, we should consider a more realistic snow albedo    !
      ! model that takes snow age and snow melt into account.                              !
      !                                                                                    !
      !  Potential starting points:                                                        !
      !                                                                                    !
      !  Roesch, A., et al., 2002: Comparison of spectral surface albedos and their impact !
      !      on the general circulation model simulated surface climate.  J. Geophys.      !
      !      Res.-Atmosph., 107(D14), 4221, 10.1029/2001JD000809.                          !
      !                                                                                    !
      !  Oleson, K.W., et al., 2010: Technical description of version 4.0 of the Community !
      !      Land Model (CLM). NCAR Technical Note NCAR/TN-478+STR.                        !
      !                                                                                    !
      !------------------------------------------------------------------------------------!
      ...
      do k = ksn,1,-1
         !---------------------------------------------------------------------------------!
         !      Fractrans is fraction of shortwave entering each sfcwater layer that gets  !
         ! transmitted through that layer.                                                 !
         !---------------------------------------------------------------------------------!
   ...
   if (cohort_count > 0) then
      ...
      case (1)
         !---------------------------------------------------------------------------------!
         !      Multiple-scatter model.  Here there is one important difference: we do NOT !
         ! scale longwave radiation, and we do NOT split longwave radiation coming from    !
         ! the sky and coming from the surface, as they interact in the middle layers.  We !
         ! save all radiation in the "incid" arrays, and scale them after the solution, so !
         ! the code called after this step can be used the same way for both radiations.   !
         !---------------------------------------------------------------------------------!
      ...
      if (twilight) then
         !---------------------------------------------------------------------------------!
         !     Solve the short-wave.  Decide which canopy radiation method we are going to !
         ! use.  Unlike the long-wave, here we scale radiation the same way in both cases, !
         ! because PAR and NIR are truly independent bands.                                !
         !---------------------------------------------------------------------------------!
         select case (icanrad)
         case (0)
            !------------------------------------------------------------------------------!
         ...
         !---------------------------------------------------------------------------------!
         !    Since there is no horizontal competition, assuming that the maximum possible !
         ! PAR is just the PAR from the tallest resolvable cohort is good enough.          !
         !---------------------------------------------------------------------------------!
         if (tuco_leaf /= 0) then
      ...
      !------------------------------------------------------------------------------------!
      !     Absorption rates of PAR, rshort, and rlong of the vegetation.  Here we check   !
      ! again whether we are solving big leaf or size- and age-structure.                  !
      !------------------------------------------------------------------------------------!
      select case (ibigleaf)
      case (0)
         !---------------------------------------------------------------------------------!
         ...
         do ico = cpatch%ncohorts,1,-1
            if (cpatch%leaf_resolvable(ico) .or. cpatch%wood_resolvable(ico)) then
               ...
               ! average between the area and absorptance.  We must treat the visible and  !
               ! near infrared separately.                                                 !
               !---------------------------------------------------------------------------!
               wleaf_vis = sngloff ( ( clumping_factor(ipft)                               &
                                     * (1.d0 - leaf_scatter_vis(ipft))                     &
      ...
      case (1)
         !---------------------------------------------------------------------------------!
         !     Big leaf solver.  Radiation flux is an extensive variable since its units   !
         ! are J/m2/s.  Therefore we just need to add the fluxes back.  For the light      !
         ! levels, we cheat and assign the value in the middle.                            !
         !---------------------------------------------------------------------------------!
         if (cpatch%leaf_resolvable(1) .or. cpatch%wood_resolvable(1)) then
            ...
            !------------------------------------------------------------------------------!
            !    Light profile: we do not split into leaves and branches.  Since this is a !
            ! big leaf, we only save the values beneath the stack.                         !
            !------------------------------------------------------------------------------!
      ...
      upward_par_above_diffuse        = albedo_ground_par * par_diff_norm
      upward_nir_above_diffuse        = albedo_ground_nir * nir_diff_norm
      upward_rshort_above_diffuse     = upward_par_above_diffuse + upward_nir_above_diffuse
