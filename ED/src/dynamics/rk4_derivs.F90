...
subroutine leaf_derivs(initp,dinitp,csite,ipa,dt)
   ...
   real(kind=8)                    :: dt        ! Current time step if euler/hybrid
...
subroutine canopy_derivs_two(mzg,initp,dinitp,csite,ipa,hflxgc,wflxgc,qwflxgc,dewgndflx    &
   ...
   real(kind=8)                     :: dt                ! Timestep if euler/hybrid
   !----- Functions -----------------------------------------------------------------------!
         ...
         if (wflxlc_try >= 0.d0) then  
            ...
            if (supersat_ok .or. initp%can_rhv < 1.d0) then
               ...
               if(dt>-8000.d0) then
                  ...
                  ! If we ever have shedding, force wshed
                  ! to cap out at that maximum leaf water
                  ! Assume this process happens before evaporation
                  
                  wshed  = max(0.d0,((initp%leaf_water(ico)                             &
...
!                  wshed = 0.0
                  ...
                  qwshed = wshed                                                       &
                       ...
                       * (initp%leaf_temp(ico) - tsupercool_liq8)                     &
