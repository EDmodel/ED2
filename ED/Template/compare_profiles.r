...
lai.defbrk   =     c(  1.5,  2.0,  2.5,  3.0,  3.5,  4.5,  6.0,  8.0)
report.solar = rbind(c(  NA ,  NA , 22.0,  8.9,  5.4,  NA ,  NA )
                    ,c(  NA ,  NA , 45.4, 11.3,  6.7,  NA ,  NA )
                    )#end rbind
report.par   = rbind(c(  NA ,  NA , 10.8,  2.6, 0.35,  NA ,  NA )
                    ,c(  NA ,  NA , 22.2,  3.3, 1.04,  NA ,  NA )
                    )#end rbind
report.nir   = rbind(c(  NA ,  NA ,  NA ,  NA ,  NA ,  NA ,  NA )
...
#------------------------------------------------------------------------------------------#
sites      = list()
sites[[1]] = list(iata="gyf",desc="Paracou"       ,pch= 2,col="#520485",year=2004)
sites[[2]] = list(iata="s67",desc="Santarem km 67",pch= 5,col="#46FF32",year=2003)
sites[[3]] = list(iata="s83",desc="Santarem km 83",pch= 9,col="#FF5700",year=2001)
sites[[4]] = list(iata="pdg",desc="Pe-de-Gigante" ,pch=13,col="#A00014",year=2003)
sites[[5]] = list(iata="rja",desc="Rebio Jaru"    ,pch= 1,col="#006715",year=2001)
sites[[6]] = list(iata="m34",desc="Manaus K34"    ,pch= 6,col="#0742C3",year=2004)
sites[[7]] = list(iata="pnz",desc="Petrolina"     ,pch= 4,col="#B49ED2",year=2004)
sites[[8]] = list(iata="ban",desc="Bananal"       ,pch= 8,col="#F5C858",year=2005)
use.sites  = 1:6
#------------------------------------------------------------------------------------------#
...
plot.site  = c(FALSE,TRUE)[1]
plot.patch = c(FALSE,TRUE)[2]
dlai       = 0.25
#------------------------------------------------------------------------------------------#
...
simul[[ 3]] = list( suff           = "icanrad01_crown00"
                  , desc           = "Zhao and Qualls (Crown OFF)"
                  , colour         = "#46FF32"
                  , mult.beam.down = 1.03
                  , mult.diff.down = 1.
                  , mult.up        = 1.
...
tsize = plotsize(proje=FALSE,stdheight=11,stdwidth=14)
#------------------------------------------------------------------------------------------#
...
if (plot.patch){
   ...
   #----- Organise the reported values. ---------------------------------------------------#
   ybottom = c(rbind(report.solar[1,],report.par[1,],report.nir[1,]))*0.01
   ytop    = c(rbind(report.solar[2,],report.par[2,],report.nir[2,]))*0.01
   #---------------------------------------------------------------------------------------#
   ...
   for (f in sequence(nflux)){
      ...
      for (o in sequence(nout)){
         ...
         for (s in sequence(nsimul)){
            ...
            #------------------------------------------------------------------------------#
            if (flux.key %in% c("down.tot") && ! add.others){
            ...
            axis.rt(side=1,las=5,at=lai.at,labels=lai.labels,off=0.1)
            box()
...
if (plot.site){
   ...
   for (p in sequence(nsites)){
      ...
      #------------------------------------------------------------------------------------#
      for (l in seq_along(model$lai.plot)){
         pnow      = model$lai.plot[l]
         lai.key   = c("laimax","laimed","laimin")[l]
         lai.desc  = c("Greenest","Medium","Brownest")[l]
         pp        = model$ppaco == pnow
                  ...
                  if (max(down.tot) > 1.){
                     ...
                     for (o in sequence(nout)){
                        ...
                        if (! bnd %in% "tir"){
                           ...
                           plot.new()
                           plot.window(xlim=down.lim,ylim=tai.lim)
                           ...
                           for (s in sequence(nsimul)){
                              lines(x=down.beam[s,],y=-cum.tai[s,]
                        ...
                        for (s in sequence(nsimul)){
                           lines(x=down.diff[s,],y=-cum.tai[s,]
                                ,col=simul$colour[s],lwd=2.5,lty="solid")
                        }#end for
                        #------------------------------------------------------------------#
                        ...
                        plot.new()
                        plot.window(xlim=up.lim,ylim=tai.lim)
                        axis(side=1,cex.axis=0.8,padj=-0.8)
                        axis(side=2,at=tai.at,labels=tai.lab,las=1)
                        grid(col=grid.colour,lwd=1.0,lty="solid")
                        box()
                        ...
                        plot.new()
                        plot.window(xlim=abs.lim,ylim=tai.lim)
                        axis(side=1,cex.axis=0.8,padj=-0.8)
                        axis(side=2,at=tai.at,labels=tai.lab,las=1)
                        grid(col=grid.colour,lwd=1.0,lty="solid")
                        box()
                        title(main="Cumulative absorption (dashed = leaves)"
                             ,font.main=1,line=1)
                        for (s in sequence(nsimul)){
                           lines(x=abs.leaf[s,],y=-cum.tai[s,]
                                ,col=simul$colour[s],lwd=2.0,lty="dotdash")
                           lines(x=abs.tot [s,],y=-cum.tai[s,]
                                ,col=simul$colour[s],lwd=2.5,lty="solid")
                        }#end for
                        #------------------------------------------------------------------#
