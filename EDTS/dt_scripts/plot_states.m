function plot_states(flux_tp,flux_cp,flux_td,flux_cd,flux_tm,flux_cm,varnames,varunits...
                    ,phrs,titlestr,fluxes_img,status_tp,status_cp,visible)

global fasz;
global t_colour;
global c_colour;

leg_text=sprintf('Test (%s): o       Main (%s): +',status_tp,status_cp);
...
set(gcf,'PaperPositionMode','manual','Units','inches','PaperSize',[11.0 8.5]...
   ,'Position',[0.25 0.25 10.5 6]);
...
for ip=1:ntmp
    ...
    hold on;
    plot(phrs,flux_td(:,ip),'o','Color',t_colour);
    plot(phrs,flux_cd(:,ip),'+','Color',c_colour);
    ...
    ax3 = axes;
    set(ax3,'Position',[bx+pos*dx+pos*mx by+2*my+2*dy dx dy],'FontSize',fasz_l);
    hold on;
    plot(1:12,flux_tm(:,ip),'o','Color',t_colour);
    plot(1:12,flux_cm(:,ip),'+','Color',c_colour);
    hold off;
    xlim([1 12]);
    set(gca,'XTick',[3 6 9],'XTicklabel',{'Mar','Jun','Sep'},'FontSize',fasz_l);
    grid on;
    box on;
    if(ip==ntmp);ylabel('Monthly Means');end;
...
axis off;
text(0.5,0.05,leg_text,'HorizontalAlignment','center','FontSize',fasz_l);
