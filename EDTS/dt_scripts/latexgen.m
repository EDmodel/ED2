...
verifications = [verifications,sprintf('The following simulations have the following status: \\\\[0.5cm] \n\n')];

verifications = [verifications,sprintf('RUN~ \\quad DBUG \\quad TEST \\quad  MAIN \\\\ \n\n')];

for is=1:nsite
    verifications = [verifications,sprintf('%s: \\quad %s ',siteid{is},runstat{spass(is,1)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s ',runstat{spass(is,2)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s \\\\ \n',runstat{spass(is,3)+1})]; %#ok<AGROW>
end

for ih=1:nhifr
    verifications = [verifications,sprintf('%s: \\quad %s ',hifrid{ih},runstat{hpass(ih,1)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s ',runstat{hpass(ih,2)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s \\\\ \n',runstat{hpass(ih,3)+1})]; %#ok<AGROW>
end

for ig=1:ngrid
    verifications = [verifications,sprintf('%s: \\quad %s ',gridid{ig},runstat{gpass(ig,1)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s ',runstat{gpass(ig,2)+1})]; %#ok<AGROW>
    verifications = [verifications,sprintf('\\quad %s \\\\ \n',runstat{gpass(ig,3)+1})]; %#ok<AGROW>
end
